...
@dg.op(ins={"met_zone": dg.In(), "agebs_2020": dg.In(dagster_type=dg.Nothing)})
def load_census(postgres_resource: PostgresResource, met_zone: str) -> pd.DataFrame:
...
def load_census_geometries(
    postgres_resource: PostgresResource, met_zone: str
) -> gpd.GeoDataFrame:
    with postgres_resource.connect() as conn:
